                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('admin.TambahDataDetailPerusahaanPesanProduk') }}"
                                            ...
                                            @csrf
                                            <input type="hidden" name="pesan_produk_id" value="{{ $pesan_produk->id }}"
                                                hidden>
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td><input class="form-control" type="n umber"
                                                                        name="stok[]" placeholder="Masukkan Jumlah"></td>
                                                                <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#tambah', function() {
                ...
                html +=
                    "<td><input  class='form-control' type='number' name='stok[]' placeholder='Masukkan Jumlah'></td>"
                html +=
        ...
        let pesan_produk = @json($pesan_produk);
        let daftar_data_perusahaan_pesan_produk = [];
        const table_data_perusahaan_pesan_produk = $('#table-data-detail-permohonan-pesan-produk').DataTable({
            "destroy": true,
            "pageLength": 10,
            "lengthMenu": [
                [10, 25, 50, 100, -1],
            ...
            "sScrollXInner": "100%",
            ajax: {
                url: "/admin/data-detail-pesan-produk/" + pesan_produk.id,
                type: "POST",
                headers: {
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        ...
                        tampilan = `
                            <div class="ms-auto">
                                <a class="btn btn-link text-dark text-gradient px-3 mb-0 edit_detail_data_perusahaan_pesan_produk" id-detail-produk-perusahaan-pesan-produk = "${row.id}" href="#!" ><i class="fas fa-pencil-alt text-dark me-2" aria-hidden="true"></i>Ubah</a>
                                <a class="btn btn-link text-danger text-gradient px-3 mb-0 hapus_detail_data_perusahaan_pesan_produk" id-detail-produk-perusahaan-pesan-produk = "${row.id}" href="#!"><i class="fa fa-trash-alt me-2"></i>Hapus</a>
                                </div>
                                `
        ...
        $(document).on('click', '.edit_detail_data_perusahaan_pesan_produk', function(event) {
            ...
            $("#formEditDetailPerusahaanPesanProduk [name='nama_produk']").val(data_perusahaan_pesan_produk
                .nama_produk)
            $("#formEditDetailPerusahaanPesanProduk [name='stok']").val(data_perusahaan_pesan_produk.stok)
            $("#formEditDetailPerusahaanPesanProduk [name='catatan']").val(data_perusahaan_pesan_produk.catatan)
        ...
        $(document).on('click', '.hapus_detail_data_perusahaan_pesan_produk', function(event) {
            const id = $(event.currentTarget).attr('id-detail-produk-perusahaan-pesan-produk');
            swal({
                title: "Yakin ?",
                text: "Menghapus Data ?",
